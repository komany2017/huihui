...
$reloadScript = Join-Path $root 'reload-node.ps1'
...
# ---------- 1. 准备 win-acme ----------
# 优先用 -WacsExe 传入的路径（用户已下载）；其次 tools\win-acme\wacs.exe；最后下载最新版
if ($WacsExe -and (Test-Path $WacsExe)) {
  $wacsExe = (Resolve-Path $WacsExe).Path
  Ok "使用外部 wacs.exe: $wacsExe"
} elseif (Test-Path $wacsExe) {
...
# win-acme v2.2+ 参数: --source（非 --target）, --emailaddress（非 --email）
$wacsArgs = @(
  '--source', 'manual',
  '--host', $Domain,
...
# wacs 可能把诊断输出到 stderr，临时切 EAP 避免被 PS 当成错误终止
...
if ($wacsExit -ne 0) {
  Warn "wacs.exe 退出码: $wacsExit"
  Write-Host "    提示: 新版 win-acme 参数可能变化，可手动跑 wacs.exe --help 查看参数列表" -ForegroundColor Gray
  Fail "wacs.exe 申请失败，请把上面输出完整贴回来"
}
...
Write-Host "  续期:       wacs 已创建/更新计划任务自动续期（默认 60 天周期，任务名 wacs）"
Write-Host '  下一步:'
