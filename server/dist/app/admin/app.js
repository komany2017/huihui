...
let role = localStorage.getItem('yyt_admin_role') || 'admin'
...
async function api(path, method = 'GET', body) {
  ...
  if (res.status === 403) throw new Error(json.msg || '访客账号仅支持浏览')
...
async function login() {
  ...
  try {
    ...
    localStorage.setItem('yyt_admin_role', role)
    applyRole()
    enterMain()
  } catch (e) {
...
function logout() {
  ...
  localStorage.removeItem('yyt_admin_role')
  applyRole()
  $('#mainView').classList.add('hidden')
  $('#loginView').classList.remove('hidden')
}

/** 按角色切换界面：guest 隐藏一切编辑入口 */
function applyRole() {
  document.body.classList.toggle('guest', role === 'guest')
  const badge = $('#roleBadge')
  if (badge) {
    badge.textContent = role === 'guest' ? '访客（只读）' : '管理员'
    badge.classList.toggle('guest-badge', role === 'guest')
  }
}
